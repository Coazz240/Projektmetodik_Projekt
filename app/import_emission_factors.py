...
def upsert_factor(db: Session, category: str, key: str, unit: str, co2e_per_unit: float, source: str | None, scope: str | None) -> None: #la till scope: str | None
    ...
    try:
        with DATA_PATH.open("r", encoding="utf-8") as f:
            ...
            for row in reader:
                ...
                scope = (row.get("scope") or "").strip() or "direct" 
                upsert_factor(db, category, key, unit, co2e_per_unit, source, scope)
                count += 1
                required = ("category", "key", "unit", "co2e_per_unit", "scope")
